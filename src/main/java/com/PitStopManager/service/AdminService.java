...
public class AdminService {
    ...
    public void updateDriver(DriverTeam driver) {
        String sql = "UPDATE driver_team SET DriverName = ?, Age = ?, MarketValue = ?, TeamID = ? WHERE DriverID = ?";
        try (Connection conn = DbConfig.getDbConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, driver.getDriverName());
            stmt.setInt(2, driver.getAge());
    ...
    public void createTeam(Team team) {
        String sql = "INSERT INTO team (TeamName, TeamPrincipal, BaseLocation, TeamBudget) VALUES (?, ?, ?, ?)";
        try (Connection conn = DbConfig.getDbConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, team.getTeamName());
            stmt.setString(2, team.getTeamPrincipal());
            stmt.setString(3, team.getBaseLocation());
            stmt.setDouble(4, team.getTeamBudget());
            stmt.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void updateTeam(Team team) {
        String sql = "UPDATE team SET TeamName = ?, TeamPrincipal = ?, BaseLocation = ?, TeamBudget = ? WHERE TeamID = ?";
        try (Connection conn = DbConfig.getDbConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, team.getTeamName());
            stmt.setString(2, team.getTeamPrincipal());
    ...
    public void createContract(Contract contract) {
        String sql = "INSERT INTO contract (DriverID, StartDate, EndDate, TransferFee) VALUES (?, ?, ?, ?)";
        try (Connection conn = DbConfig.getDbConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, contract.getDriverId());
            stmt.setDate(2, java.sql.Date.valueOf(contract.getStartDate()));
            stmt.setDate(3, java.sql.Date.valueOf(contract.getEndDate()));
            stmt.setDouble(4, contract.getTransferFee());
            stmt.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void updateContract(Contract contract) {
        String sql = "UPDATE contract SET DriverID = ?, StartDate = ?, EndDate = ?, TransferFee = ? WHERE ContractID = ?";
        try (Connection conn = DbConfig.getDbConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, contract.getDriverId());
            stmt.setDate(2, java.sql.Date.valueOf(contract.getStartDate()));
            stmt.setDate(3, java.sql.Date.valueOf(contract.getEndDate()));
            stmt.setDouble(4, contract.getTransferFee());
            stmt.setInt(5, contract.getContractId());
            stmt.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
